                    ...
                    let lonelyListButton = $('<a href="#lonely-objects-page" id="lonely-objects-button">OGGETTI SPARSI</a>').on('tap', function () {
                        ...
                        setInterval(function () {
                            $.ajax({
                                ...
                                // console.log('updating objects: ' + i++);
                                let lonelyObjects  = $('.lonely-objects-list');
                                let lostJsonResult = JSON.parse(lost);
                                let createKit      = false;
                                lonelyObjects.empty();

                                //gestisco gli oggetti sparsi
                                $.each(lostJsonResult[0], function (innerKey, innerValue) {
                                    ...
                                    if (innerValue['kit_create'] === 1) {
                                        ...
                                        if (checkedBoxes[innerValue['cod']] === '1') {
                                            lonelyObjects.append('<li class="lonely-object" id="' + innerValue['cod'] + '"><div class="flot-left width-100"><span class="float-left">'
                                                + innerValue['name'] + '</span><span class="float-right margin-right-30px"><input type="checkbox" id="' + innerValue['cod'] + '-check" class="checkAntani" checked="checked"></span></div></li>').trigger('create');
                                        } else {
                                            lonelyObjects.append('<li class="lonely-object" id="' + innerValue['cod'] + '"><div class="flot-left width-100"><span class="float-left">'
                                                + innerValue['name'] + '</span><span class="float-right margin-right-30px"><input type="checkbox" id="' + innerValue['cod'] + '-check" class="checkAntani"></span></div></li>').trigger('create');
                                        }
                                        ...
                                        let createKitButton = $('<a href="#" id="create-kit" class="ui-btn ui-shadow ui-corner-all create-kit-button margin-auto font-large">CREA KIT</a>').on('click', function () {
                                            ...
                                            if ($('#kit-name input').val() === '')
                                                ...
                                                if (count > 0) {
                                                    ...
                                                    createKitForm.append('time', time);
                                ...
                                $.each(lonelyObjects.children(), function (key, value) {
                                    $('#' + $(value).attr('id') + '-check').change(function () {
                                        if ($(this).prop('checked')) {
                                            checkedBoxes[$(value).attr('id')] = '1';
                                        } else {
                                            checkedBoxes[$(value).attr('id')] = '0';
                                        }
                                    })
                                });
                            })
